...
class Cliente:
    ...
    def cargarTurno(self):
        ...
        while not turno_valido:
            def horarios(numeroTurno, turno):
                print(
                ...
                if turno == 'm':
                    print(
                        "Elegiste el turno MAÑANA, tenes dos opciones para atenderte..\nIngresá 1 para el primer turno, de 10 a 12hs.\nIngresá 2 para el segundo turno, de 12 a 14hs.")
                    numeroTurno = int(input())

                    if numeroTurno == 1:
                        print("Felicitaciones, tenés el " + str(numeroTurno) + "° turno el día de 10 a 12hs.")
                    elif numeroTurno == 2:
                        print("Felicitaciones, tenés el " + str(numeroTurno) + "° turno el día de 12 a 14hs.")

                elif turno == 't':
                    print(
                        "Elegiste el turno TARDE,tenés tres opciones para atenderte..\nIngresá 1 para el primer turno, de 14 a 16hs.\nIngresá 2 para el segundo turno, de 16 a 18hs.\nIngresá 3 para el tercer turno, de 18 a 20hs.")
                    numeroTurno = int(input())

                    if numeroTurno == 1:
                        print("------------------------------------------------------------------------------")
                        print("Felicitaciones, tenés el " + str(numeroTurno) + "° turno asignado el día de 14 a 16hs.")
                        print("------------------------------------------------------------------------------")
                    elif numeroTurno == 2:
                        print("------------------------------------------------------------------------------")
                        print("Felicitaciones, tenés el " + str(numeroTurno) + "° turno asignado el día de 16 a 18hs.")
                        print("------------------------------------------------------------------------------")
                    elif numeroTurno == 3:
                        print("------------------------------------------------------------------------------")
                        print("Felicitaciones, tenés el " + str(numeroTurno) + "° turno asignado el día de 18 a 20hs.")
                        print("------------------------------------------------------------------------------")
                    else:
                        print(
            ...
            # Realizar la consulta para verificar si el turno está ocupado
            # Consulta para verificar mes y dia
            columna_turno = f"turno{turno}"
            cursor.execute(f"SELECT {columna_turno} FROM Perri_Centro_Spa.turnos WHERE dia=%s AND mes=%s",
                           (self.dia, self.mes))
            # Falta consulta para verificar turno(mañana o tarde) y numero de turno(1,2 o 1,2,3)
